    ...
    int userId)
{
    ...
    try
    {
        ...
        for (const auto& row : result)
        {
            ...
            item["price_cents"] = row["price_cents"].as<int>();
            item["quantity"] = row["quantity"].as<int>();
            const auto subtotal = row["subtotal_cents"].as<long long>();
    ...
    int userId)
{
    ...
    try
    {
        ...
        const int productId = (*body)["product_id"].asInt();
        const int quantity = (*body)["quantity"].asInt();
        if (productId <= 0 || quantity <= 0)
        {
            json["success"] = false;
            json["error"] = "product_id and quantity must be positive";
            auto response =
                drogon::HttpResponse::newHttpJsonResponse(json);
            response->setStatusCode(drogon::k400BadRequest);
            callback(response);
        ...
        const int stockQty = product[0]["stock_qty"].as<int>();
        ...
        const int existingQty =
            existing.empty() ? 0 : existing[0]["quantity"].as<int>();
    ...
    catch (const std::exception& e)
    {
        ...
        LOG_ERROR << "addToCart error: " << e.what();
        auto response = drogon::HttpResponse::newHttpJsonResponse(json);
        response->setStatusCode(drogon::k500InternalServerError);
        callback(response);
    }
    ...
    int productId)
{
    ...
    {
        json["success"] = false;
        json["error"] = "Invalid user ID or product ID";
        auto response = drogon::HttpResponse::newHttpJsonResponse(json);
        response->setStatusCode(drogon::k400BadRequest);
        callback(response);
        return;
    ...
    {
        const auto body = request->getJsonObject();
        if (!body || !body->isMember("quantity"))
        {
            json["success"] = false;
        ...
        json["success"] = true;
        json["message"] = "Cart item updated";
        callback(drogon::HttpResponse::newHttpJsonResponse(json));
    }
    catch (const std::exception& e)
    {
        json["success"] = false;
        json["error"] = "Failed to update cart item";
        LOG_ERROR << "updateCartItem error: " << e.what();
        auto response = drogon::HttpResponse::newHttpJsonResponse(json);
        response->setStatusCode(drogon::k500InternalServerError);
        callback(response);
    }
    ...
    int productId)
{
    ...
    {
        json["success"] = false;
        json["error"] = "Failed to remove cart item";
        LOG_ERROR << "removeFromCart error: " << e.what();
        auto response = drogon::HttpResponse::newHttpJsonResponse(json);
        response->setStatusCode(drogon::k500InternalServerError);
        callback(response);
    }
